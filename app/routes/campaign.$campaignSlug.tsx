...
export const loader = async ({ params }: LoaderFunctionArgs) => {
    ...
    const campaignChapter = CampaignChapters.find(
        (cc) => cc.slug === params.campaignSlug
    );
...
export default function Campaign() {
    ...
    return (
        <div
            id={`campaign-${campaignChapter.slug}`}
            className="flex space-x-4 space-y-8 flex-col"
        >
            <h2 className="text-2xl font-semibold">
                {campaignChapter.chapter}-{campaignChapter.level}:{" "}
                {campaignChapter.name}
            </h2>
            <div className="text-gray-800 italic">
                {campaignChapter.description}
            </div>
            <div>Energy cost: {campaignChapter.energy_cost}</div>
            <div>
                Hero soul stone:{" "}
                {campaignChapter.hero ? (
                    <span className="underline decoration-dotted underline-offset-4 italic cursor-not-allowed">
                        {campaignChapter.hero}
                    </span>
                ) : (
                    "(none)"
            ...
            {campaignChapter.gold && (
                <ul>
                    <li>Gold: {campaignChapter.gold}</li>
            ...
            {campaignChapter.hero_exp && (
                <ul>
                    <li>Hero Exp: {campaignChapter.hero_exp}</li>
                    {campaignChapter.energy_cost && (
                        <li>
                            Hero Exp/Energy:{" "}
                            {Math.round(
