...
export async function initializeEquipmentBlobs(options: InitializationOptions = {}): Promise<InitializationResult> {
  ...
  try {
    ...
    const store = getStore({ name: "equipment" });
    ...
    if (existingCount > 0) {
      ...
      if (failIfExists) {
        throw new Error("Existing equipment data found. Aborting initialization.");
      }
    }
    ...
    for (const equipment of equipmentData as unknown as EquipmentRecord[]) {
      try {
        ...
        if (exists && !forceUpdate) {
          if (skipExisting) {
            details.push(`⤍ Skipping existing item: ${equipment.name} (${equipment.slug})`);
            skippedCount++;
            continue;
          } else {
            throw new Error("Item already exists and skipExisting is false");
          }
        }
        ...
        details.push(`✓ Successfully ${action.toLowerCase()} ${equipment.name} (${equipment.slug})`);
        successCount++;
      } catch (error) {
